...
/// 					Esta função cria um processo filho para executar o comando representado
/// 					pelos argumentos `argv` usando a chamada de sistema execve. O processo pai
/// 					espera pela conclusão do processo filho e, opcionalmente, verifica o status
/// 					de saída.
...
int pipex(t_script *s, char **path_env) 
{
    ...
    } else if (child_pid == 0)
	{
        ...
        execute_do_command(s->commands[0].argv, path_env, STDIN_FILENO, pipe_fd[1]);
        exit(EXIT_SUCCESS);
    } 
	else 
	{
        ...
        execute_do_command(s->commands[1].argv, path_env, pipe_fd[0], STDOUT_FILENO);
